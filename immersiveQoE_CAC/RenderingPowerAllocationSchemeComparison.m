...
fid = fopen('dados_renderizacao.csv', 'w');
fprintf(fid, 'uniforme, random, attention, otimal\n');
for u = 1:30
    fprintf(fid, '%f, %f, %f, %f\n', FINAL(u,2), FINAL(u,1), FINAL(u,3), FINAL(u,4));
end
fclose(fid);
...
% Adicionando rótulos com os valores de Diff1 acima de cada barra
hold on;
for i = 1:size(FINAL, 1)
    %text(i, FINAL(i, 2) + 1, [num2str(Diff1(i)*100, '%.2f'), '%'], 'HorizontalAlignment', 'center', 'Color', 'red');
    text(i, FINAL(i, 2) + 10, [num2str(Diff1(i)*100, '%.2f'), '%'], 'HorizontalAlignment', 'center', 'Color', 'red');
end

% Adicionando rótulos com os valores de Diff2 acima de cada barra
for i = 1:size(FINAL, 1)
    text(i, FINAL(i, 3) + 20, [num2str(Diff2(i)*100, '%.2f'), '%'], 'HorizontalAlignment', 'center', 'Color', 'blue');
end
hold off;
